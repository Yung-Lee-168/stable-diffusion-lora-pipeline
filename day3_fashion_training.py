...
class FashionTrainingPipeline:
    ...
    def calculate_image_similarity(self, generated_img, source_img):
        """計算生成圖片與原圖的相似度 - 使用與 analyze_results.py 完全相同的方法"""
        print("📊 計算圖片相似度...")
        
        similarities = {}
        
        try:
            # 🎯 統一實現 1: SSIM 結構相似度 (與 analyze_results.py 完全一致)
            gen_array = np.array(generated_img)
            src_array = np.array(source_img)
            
            # 轉換為灰階 (BGR→GRAY, 與 analyze_results.py 一致)
            gen_gray = cv2.cvtColor(gen_array, cv2.COLOR_RGB2GRAY)
            src_gray = cv2.cvtColor(src_array, cv2.COLOR_RGB2GRAY)
            
            # 確保尺寸一致 (使用較小尺寸，與 analyze_results.py 一致)
            if gen_gray.shape != src_gray.shape:
                target_shape = (min(gen_gray.shape[0], src_gray.shape[0]), 
                              min(gen_gray.shape[1], src_gray.shape[1]))
                gen_gray = cv2.resize(gen_gray, (target_shape[1], target_shape[0]))
                src_gray = cv2.resize(src_gray, (target_shape[1], target_shape[0]))
            
            # 🎯 使用 skimage.metrics.ssim (與 analyze_results.py 完全一致)
            from skimage.metrics import structural_similarity as ssim
            ssim_score = ssim(gen_gray, src_gray)
            similarities["visual_ssim"] = float(ssim_score)
            
            # 🎯 統一實現 2: 色彩分布相似度 (與 analyze_results.py 完全一致)
            # 轉換為 RGB (與 analyze_results.py 一致)
            gen_rgb = np.array(generated_img)  # 已經是 RGB
            src_rgb = np.array(source_img)     # 已經是 RGB
            
            # 計算 RGB 直方圖 (32×32×32, 與 analyze_results.py 一致)
            gen_hist = cv2.calcHist([gen_rgb], [0, 1, 2], None, [32, 32, 32], [0, 256, 0, 256, 0, 256])
            src_hist = cv2.calcHist([src_rgb], [0, 1, 2], None, [32, 32, 32], [0, 256, 0, 256, 0, 256])
            
            # 🎯 正規化步驟 (與 analyze_results.py 一致)
            gen_hist = cv2.normalize(gen_hist, gen_hist).flatten()
            src_hist = cv2.normalize(src_hist, src_hist).flatten()
            
            # 計算相關係數 (與 analyze_results.py 一致)
            color_similarity = cv2.compareHist(gen_hist, src_hist, cv2.HISTCMP_CORREL)
            similarities["color_distribution"] = float(color_similarity)
            
            # 🎯 統一實現 3: FashionCLIP 標籤匹配相似度 (與 analyze_results.py 完全一致)
            # 使用與預處理、推理、評估完全相同的標籤匹配方法
            fashion_similarity = self._calculate_fashionclip_label_similarity(generated_img, source_img)
            similarities["fashion_clip"] = float(fashion_similarity)
            print(f"   ✅ FashionCLIP 標籤匹配相似度: {fashion_similarity:.3f}")
            
            # 標準 CLIP 已完全禁用
            similarities["standard_clip"] = 0.0
            
            print(f"📊 統一相似度分數 (全流程標籤匹配一致性):")
            print(f"   視覺結構 (SSIM): {similarities.get('visual_ssim', 0):.3f}")
            print(f"   🎯FashionCLIP 標籤匹配: {similarities.get('fashion_clip', 0):.3f}")
            print(f"   色彩分布: {similarities.get('color_distribution', 0):.3f}")
            print(f"   🚫 標準 CLIP: 已禁用")
            print(f"   ✅ 與 analyze_results.py 完全一致的評估方法")
            ...
            similarities = {
                "visual_ssim": 0.5,
                "fashion_clip": 0.5,
                "standard_clip": 0.0,  # 已禁用，設為0
                "color_distribution": 0.5
            }
        
        return similarities
    
    def _calculate_fashionclip_label_similarity(self, generated_img, source_img):
        """
        計算 FashionCLIP 標籤匹配相似度 - 與 analyze_results.py 的 compare_fashion_features 完全一致
        使用與預處理、LoRA訓練、推理、評估相同的標籤匹配方法
        """
        try:
            if not self.fashion_clip_model or not self.fashion_clip_processor:
            ...
            # 1. 對生成圖片進行特徵分析 (使用相同的 extract_fashion_features 方法)
            # 保存生成圖片為臨時文件以便分析
            import tempfile
            with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as temp_gen:
                generated_img.save(temp_gen.name)
                gen_analysis = self.extract_fashion_features(temp_gen.name)
                os.unlink(temp_gen.name)  # 清理臨時文件
            
            # 2. 對原始圖片進行特徵分析 (使用相同的 extract_fashion_features 方法)
            with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as temp_src:
                source_img.save(temp_src.name)
                src_analysis = self.extract_fashion_features(temp_src.name)
                os.unlink(temp_src.name)  # 清理臨時文件
            
            # 3. 使用與 analyze_results.py 完全相同的比較邏輯
            similarities = []
            
            # 比較每個類別 (與 analyze_results.py 的 compare_fashion_features 一致)
            for category in src_analysis.keys():
                if category in gen_analysis:
                    # 獲取最佳標籤和置信度
                    if 'top_labels' in src_analysis[category] and 'top_labels' in gen_analysis[category]:
                        orig_top = src_analysis[category]['top_labels'][0]  # 取第一個最佳標籤
                        gen_top = gen_analysis[category]['top_labels'][0]
                        orig_conf = src_analysis[category]['confidence']
                        gen_conf = gen_analysis[category]['confidence']
                    elif 'error' in src_analysis[category] or 'error' in gen_analysis[category]:
                        continue  # 跳過有錯誤的類別
                    else:
                        continue
                    
                    # 🎯 與 analyze_results.py 完全相同的計算公式
                    # 標籤匹配度
                    ...
                    # 綜合相似度 (0.7 * 標籤匹配 + 0.3 * 信心度相似性)
                    category_similarity = 0.7 * label_match + 0.3 * conf_similarity
                    
                    similarities.append(category_similarity)
                    
                    print(f"      {category}: {orig_top} vs {gen_top} | 匹配={label_match:.1f}, 信心度相似={conf_similarity:.3f}, 類別相似度={category_similarity:.3f}")
            
            # 4. 計算平均相似度 (與 analyze_results.py 一致)
            if similarities:
                average_similarity = sum(similarities) / len(similarities)
                print(f"   📊 FashionCLIP 標籤匹配: {len(similarities)} 個類別, 平均相似度={average_similarity:.3f}")
            else:
            ...
            print(f"   ❌ FashionCLIP 標籤匹配計算失敗: {e}")
            return 0.0

    def calculate_combined_loss(self, similarities):
        """
        計算組合損失函數 - 與 analyze_results.py 使用完全相同的公式和權重
        🎯 統一實現: total_loss = 0.2 * visual_loss + 0.6 * fashion_clip_loss + 0.2 * color_loss
        """
        weights = self.training_config["loss_weights"]
        
        # 🎯 與 analyze_results.py 完全一致的損失轉換
        ...
        # 🎯 與 analyze_results.py 完全一致的加權組合公式
        total_loss = (
            weights["visual"] * visual_loss +           # 視覺結構相似度 (權重: 0.2)
            weights["fashion_clip"] * fashion_clip_loss +  # FashionCLIP 語意相似度 (權重: 0.6, 主要)
            weights["color"] * color_loss               # 色彩分布相似度 (權重: 0.2)
        )
        
        # 📊 詳細損失分析 (與 analyze_results.py 格式一致)
        print(f"📊 統一損失分析 (與 analyze_results.py 一致):")
        print(f"   總損失: {total_loss:.4f}")
        print(f"   🎯 FashionCLIP損失: {fashion_clip_loss:.4f} (權重: {weights['fashion_clip']})")
        print(f"   👁️ 視覺SSIM損失: {visual_loss:.4f} (權重: {weights['visual']})")
        print(f"   🎨 色彩分布損失: {color_loss:.4f} (權重: {weights['color']})")
        print(f"   🚫 標準 CLIP: 已禁用")
